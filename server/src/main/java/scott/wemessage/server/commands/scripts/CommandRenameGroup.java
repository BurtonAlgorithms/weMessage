...
public class CommandRenameGroup extends ScriptCommand {
    ...
    public void execute(String[] args){
        ...
        try {
            ...
            if (args.length == 2) {
                if (chats.size() == 1){
                    Object result = getScriptExecutor().runRenameGroupScript(chats.get(0), args[1]);

                    ServerLogger.log("Action Rename Group returned a result of: " + processResult(result));
                    return;
                }
                ...
                if (chats.size() > 1){
                    ...
                    ServerLogger.log("Example Usage: renamegroup \"Group Name\" 2 \"New Group Name\"");
                ...
                Object result = getScriptExecutor().runRenameGroupScript(chat, args[2]);
                ServerLogger.log("Action Rename Group returned a result of: " + processResult(result));
            } else if (args.length > 3){
                ServerLogger.log("There were too many arguments provided. Make sure your arguments are surrounded in \"quotation marks.\"");
                ServerLogger.log("Example Usage: renamegroup \"Group Name\" \"New Group Name\"");
                ...
                ServerLogger.log("Example Usage: renamegroup \"Group Name\" 3 \"New Group Name\"");
